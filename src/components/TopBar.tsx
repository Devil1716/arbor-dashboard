...
export const TopBar: React.FC = () => {
  const { connected, battery, wifiStrength, mode, piIpAddress } = useRobotStore();
    ...
    <div className="sticky top-0 z-40 flex flex-col gap-2 p-4 bg-card border-b-2 border-black/80 shadow-panel">
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          {piIpAddress && (
            <span className="text-[10px] text-muted-foreground mr-1 border border-white/10 px-1 rounded bg-black/20">
              {piIpAddress}
            </span>
          )}
